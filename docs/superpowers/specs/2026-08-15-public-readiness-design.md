...
- 增加漏洞报告、贡献指南、合并的 Issue 表单、PR 模板和依赖更新配置。
- 将第三方 GitHub Actions 固定到不可变提交 SHA，并在公开后启用 CodeQL Default Setup。
- 稳定安装命令固定到当前最新 Release，并由 Release Please 同步升级；`main` 只作为明确标注的开发入口。
...
改写前在仓库外创建完整 bundle，并验证其中包含全部分支和标签。历史清理采用可审计的脚本一次完成：个人作者和提交者身份改为 `xuanjinchen <120694391+xuanjinchen@users.noreply.github.com>`，保留 GitHub Bot 身份；历史文档中的真实用户目录与仓库绝对路径改为 `<repo>` 或 `$HOME` 等中性占位符。
