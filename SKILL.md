...
- 新增公开、受保护或导出的具名函数或方法时，默认在声明处添加一条声明级注释，简述职责、业务处理和目标；存在必要信息时补充调用方可观察行为、边界、异常或副作用。
- 新增接口、协议、抽象基类、HTTP API、RPC API、事件或回调等外部契约时，默认添加简洁契约注释。接口契约优先写在声明处；实现只记录特有处理或差异，不重复同义文档。
- 新增公开类型仅在名称不足以表达领域或 API 契约时注释。构造器、工厂、字段、属性、配置项和错误类型仅记录不变量、单位、范围、默认值、所有权、恢复或兼容语义。
- 私有函数或方法仅当存在非直观业务意图、约束或副作用时注释。简单 getter、setter、空构造器、透明委托、无契约变化的 override、测试辅助与生成代码默认豁免；无法确认语义时不得猜测。
