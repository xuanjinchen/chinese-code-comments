...
```bash
npx --yes git+https://github.com/xuanjinchen/chinese-code-comments.git#v0.2.0 install
```
...
```bash
npx skills add xuanjinchen/chinese-code-comments#v0.2.0 -g --all
```
...
完整安装后，目标 Agent 会在实现过程中记录有维护价值的关键意图，并在结束前加载 `chinese-code-comments` 审查完整 diff 和未跟踪交付文件。新增公共方法、公开函数或外部接口时默认补充简洁的声明级契约注释；其他代码只有确实存在维护价值时才新增或更新注释。
...
| `新增公开的订单查询方法` | `SCOPED`，默认中文，方法级 | 默认说明方法职责、业务目标及必要的调用方可观察行为。 |
| `创建 PaymentGateway 接口` | `SCOPED`，默认中文，API 级 | 在接口声明处记录职责和实现者契约，不重复注释清晰的成员。 |
...
| 新增公共声明 | 公开、受保护或导出的具名函数和方法默认添加一条简洁声明级注释。 |
| 新增外部契约 | 接口、协议、抽象基类、HTTP/RPC API、事件和公开回调默认添加契约注释。 |
| 私有方法 | 仅在存在非直观业务意图、约束、副作用或其他高价值上下文时注释。 |
| 现有注释 | 保留准确的现有注释及其语言；只更新失真、错误或与代码改动冲突的注释。 |
| 注释密度 | 默认不逐句翻译赋值、循环、函数调用或其他自解释代码。 |
| 完整审查 | 实现结束前检查完整 diff 和未跟踪交付文件，不只检查新增代码行。 |
| 无需注释 | 未新增需要文档化的公共契约且没有其他维护上下文时，允许不新增注释，但仍须报告已完成审查。 |
...
| 分类 | 默认处理 |
| --- | --- |
| 公共调用单元 | 新增的公开、受保护或导出函数和方法，在声明处用最短文字说明职责、业务处理目标及必要的边界、异常或副作用。 |
| 外部契约 | 接口、协议、抽象基类、HTTP/RPC API、事件和回调在契约声明处注释；实现只补充特有处理或差异。 |
| 公开类型 | class、struct、record、enum、type alias 或 delegate 建立名称无法完整表达的领域/API 契约时注释。 |
| 条件注释 | 构造器、工厂、字段、属性、配置项和错误类型仅在不变量、单位、范围、默认值、所有权、恢复或兼容语义不直观时注释。 |
| 默认豁免 | 私有自解释方法、简单 getter/setter、空构造器、透明委托、无契约变化的 override、测试辅助和生成代码。 |

接口成员的清晰签名已经完整表达契约时，不重复添加同义注释。无法从需求、代码、测试或文档确认的业务语义不会被猜测写入源码。
...
```bash
npx --yes git+https://github.com/xuanjinchen/chinese-code-comments.git#v0.2.0 install --agent codex
npx --yes git+https://github.com/xuanjinchen/chinese-code-comments.git#v0.2.0 install --agent codex,gemini --agent hermes
```
...
```bash
npx --yes git+https://github.com/xuanjinchen/chinese-code-comments.git#v0.2.0 install
npx --yes git+https://github.com/xuanjinchen/chinese-code-comments.git#v0.2.0 install --agent claude
```
...
```bash
npx --yes git+https://github.com/xuanjinchen/chinese-code-comments.git#v0.2.0 doctor
npx --yes git+https://github.com/xuanjinchen/chinese-code-comments.git#v0.2.0 doctor --agent codex
```
...
```bash
npx --yes git+https://github.com/xuanjinchen/chinese-code-comments.git#v0.2.0 uninstall --agent codex
```

卸载全部六个 Agent：

```bash
npx --yes git+https://github.com/xuanjinchen/chinese-code-comments.git#v0.2.0 uninstall
```
...
以下命令面向仓库维护者。它们要求 Node.js 22.6+，可在 Windows、macOS 和 Linux 上运行。
...
默认 live eval 运行 8 个核心案例；`eval:full` 运行全部 24 个跨语言结构化案例；`--case` 只运行指定案例。每个案例启动一次独立模型会话，因此完整评测成本明显高于默认评测。smoke 只启动一次模型会话，并在隔离的临时 Git 仓库中发送不含“注释”字样的代码修改请求，验证自动触发行为、真实 diff、注释质量和最终审查报告；当 Agent 输出可观察的工具轨迹时，还会直接验证 Skill 读取和审查顺序，不提供工具轨迹的 Agent 则只报告行为证据。live eval 和 smoke 可能产生模型费用，且不会由 `npm run check` 或默认 CI 自动运行。
...
├── evals/evals.json             # 24 个真实模型评测定义
├── tests/                       # 单元、集成、契约、live eval 和 smoke
├── package.json                 # Node.js 22.6+ 命令与包元数据
